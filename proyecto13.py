...
class KMLProcessor:
    ...
    # --- load_kml (como en la versión anterior) ---
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for pair in coords_pairs:
                    try:
                        ...
                        # else: print(f"Warn: Coords incompleto '{pair}'") # Opcional: silenciar advertencias
                    # except ValueError: print(f"Warn: Coords inválido '{pair}'") # Opcional: silenciar advertencias
                    except ValueError: pass # Ignorar pares inválidos silenciosamente
    ...
    # --- calculate_inner_area (como en la versión anterior) ---
    ...
    # --- calculate_units (VERSIÓN CORREGIDA INTEGRADA) ---
    def calculate_units(self, base_width, base_length, corridor_width):
        ...
        long_base_dim = max(base_width, base_length) # "parte mas larga de la unidad base"

        # *** CÁLCULO DE STAIR_SIZE CORREGIDO ***
        # stair_size = (parte más larga UBase Ext) + (parte más larga UBase Int) + (dim Pasillo)
        # Como UBase Ext e Int tienen la misma "parte más larga" intrínseca:
        stair_size = (2 * long_base_dim) + corridor_width
        # Descomentar para depuración si es necesario:
        # print(f"Debug: long_base_dim={long_base_dim}, corridor_width={corridor_width}, STAIR_SIZE_CALCULATED={stair_size}")


        # Ajustar stair_size si no cabe en el inner_area
        required_dim_for_stairs = 2 * stair_size # Necesitamos espacio para dos escaleras (una a cada lado)
        if required_dim_for_stairs > self.inner_area['width'] or required_dim_for_stairs > self.inner_area['height']:
             ...
             if stair_size > available_space_for_one_stair and available_space_for_one_stair > 0 : # Añadido chequeo > 0
             ...
             elif available_space_for_one_stair <= 0: # Si no hay espacio ni para una escalera reducida
                  # Es posible que el offset ya haya hecho el inner_area muy pequeño
                  self.stair_units = [] # No se pueden crear escaleras
                  print("Advertencia: No hay espacio suficiente en el área interna para las unidades escalera, incluso reducidas.")
                  # No retornamos False aquí, intentaremos generar el resto si es posible,
                  # pero el usuario verá que no hay escaleras. O podríamos retornar False.
                  # Por ahora, permitimos continuar pero sin escaleras.
                  # Si retornamos False, el programa se detiene, lo que puede ser mejor.
                  # return False, "No hay espacio para U. Escalera. Revise Offset o Dims."

        if stair_size <= 0 : # Chequeo después de posible reducción
             return False, f"Tamaño U. Escalera calculado es <= 0 ({stair_size:.2f}). Verifique dimensiones base, pasillo y offset."


        # --- Unidades Escalera (con nuevo tamaño) ---
        # Solo crear si stair_size es positivo y hay inner_area
        ...
        else: # Si no se pudieron crear escaleras (stair_size <= 0 o no inner_area)
            self.stair_units = [] # Asegurar que esté vacío


        # --- Área Central ---
        # El cálculo del área central depende del espacio que DEJAN las escaleras.
        # Si no hay escaleras o son de tamaño 0, el espacio disponible es todo el inner_area.
        effective_stair_size_for_center = stair_size if self.stair_units else 0
        ...
        central_size_dim = min(center_avail_width, center_avail_height) * 0.4 # Mantenemos 40% del menor lado disponible
        ...
        # El punto de inicio para las unidades base es después de la unidad escalera (si existe)
        start_offset_x_bottom = self.stair_units[0]['x'] + self.stair_units[0]['width'] if self.stair_units else self.inner_area['min_x']
        end_limit_x_bottom = self.stair_units[1]['x'] if self.stair_units else self.inner_area['max_x']
        
        y_outer_bottom = self.inner_area['min_y']
        current_x = start_offset_x_bottom
        
        if current_x < end_limit_x_bottom: # Solo generar si hay espacio
        ...
        start_offset_x_top = self.stair_units[2]['x'] + self.stair_units[2]['width'] if self.stair_units else self.inner_area['min_x']
        end_limit_x_top = self.stair_units[3]['x'] if self.stair_units else self.inner_area['max_x']

        y_outer_top = self.inner_area['max_y'] - unit_h_v
        current_x = start_offset_x_top

        if current_x < end_limit_x_top: # Solo generar si hay espacio
        ...
        start_offset_y_left = self.stair_units[0]['y'] + self.stair_units[0]['height'] if self.stair_units else self.inner_area['min_y']
        end_limit_y_left = self.stair_units[2]['y'] if self.stair_units else self.inner_area['max_y']
        current_y = start_offset_y_left

        if current_y < end_limit_y_left: # Solo generar si hay espacio
        ...
        start_offset_y_right = self.stair_units[1]['y'] + self.stair_units[1]['height'] if self.stair_units else self.inner_area['min_y']
        end_limit_y_right = self.stair_units[3]['y'] if self.stair_units else self.inner_area['max_y']
        current_y = start_offset_y_right

        if current_y < end_limit_y_right: # Solo generar si hay espacio
            while current_y + unit_h_h <= end_limit_y_right + epsilon:
                ...
                corridor = {'x': x_corridor, 'y': current_y, 'width': corridor_width, 'height': unit_h_h}
                self.corridor_units.append(corridor)
                x_inner = x_corridor - unit_w_h # Debería ser - unit_w_h para la base interior
                inner_unit = {'x': x_inner, 'y': current_y, 'width': unit_w_h, 'height': unit_h_h}
                self.inner_base_units.append(inner_unit)
                current_y += unit_h_h
...
class Application(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Distribución de Unidades en Terreno v2.1") # Versión actualizada
    ...
    def calculate_and_visualize(self):
        ...
        success, message_units = self.kml_processor.calculate_units(base_width, base_length, corridor_width) # Renombrado message para evitar conflicto
    ...
    def _visualize_results(self):
        ...
        if self.kml_processor.inner_area:
            ...
            # Stair Units
            stair_plotted = False
            for unit in self.kml_processor.stair_units: # Dibujar incluso si la lista está vacía (no hará nada)
        ...
        # Manejar leyenda para que no muestre etiquetas duplicadas si listas están vacías
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles)) # Elimina duplicados manteniendo el orden de aparición
        if by_label: # Solo mostrar leyenda si hay algo que etiquetar
