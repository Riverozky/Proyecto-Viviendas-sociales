...
class KMLProcessor:
    ...
    def calculate_units(self, base_width, base_length):
        ...
        # Tamaño de las unidades escalera (usando la dimensión más grande de la unidad base)
        ...
        # Generar unidades base en los lados horizontales y verticales de las escaleras
        self.base_units = []
        
        # Generar unidades base en el lado derecho de stair1 (vertical)
        ...
        # Generar unidades base en el lado superior de stair1 (horizontal)
        x = self.stair_units[0]['x']
        y = self.stair_units[0]['y'] + stair_size
        while x + base_width <= self.stair_units[1]['x'] and y + base_length <= self.central_area['min_y']:
            self.base_units.append({
                'x': x,
                'y': y,
                'width': base_width,
        ...
        # Generar unidades base en el lado izquierdo de stair4 (vertical)
        ...
        # Generar unidades base en el lado inferior de stair4 (horizontal)
        x = self.stair_units[3]['x']
        y = self.stair_units[3]['y'] - base_length
        while x - base_width >= self.stair_units[2]['x'] and y >= self.central_area['max_y']:
            self.base_units.append({
                'x': x - base_width,
                'y': y,
                'width': base_width,
        ...
        # Generar unidades base en el lado superior de stair3 (horizontal)
        x = self.stair_units[2]['x'] + stair_size
        y = self.stair_units[2]['y']
        ...
        # Generar unidades base en el lado derecho de stair2 (vertical)
        x = self.stair_units[1]['x']
        y = self.stair_units[1]['y'] + stair_size
        while y + base_length <= self.stair_units[3]['y'] and x + base_width <= self.central_area['min_x']:
            self.base_units.append({
                'x': x,
                'y': y,
                'width': base_width,
